...
<head>
    <meta charset="UTF-8">
    <title>Career Guidance Session - ${conversation.title}</title>
    <style>
      ...
      if (message.role === 'assistant' && message.metadata) {
        ...
        content = `
          <div class="structured-section">
            ...
            <div><strong>Feasibility:</strong> ${data.careerPathSummary?.feasibility || 'Not specified'}</div>
            ${data.careerPathSummary?.alternativeRoutes?.length ? 
              `<div class="subsection-title">Alternative Routes:</div>
               ${data.careerPathSummary.alternativeRoutes.map(route => `<div class="list-item">• ${route}</div>`).join('')}` 
              : ''}
          </div>
          ...
          <div class="structured-section">
            ...
            ${data.collegesAndCourses.map(college => `
              <div class="college-item">
                <strong>${college.name}</strong> - ${college.program}<br>
                <small>📍 ${college.location} | 📝 ${college.entranceExam} | 🌐 ${college.website}</small>
              </div>
            `).join('')}
          </div>` : ''}
          ...
          <div class="structured-section">
            ...
            ${data.jobInsights.roles?.length ? 
              `<div class="subsection-title">Job Roles:</div>
               ${data.jobInsights.roles.map(role => `<div class="list-item">• ${role}</div>`).join('')}` 
              : ''}
          ...
          <div class="structured-section">
            ...
            ${data.preparationTips.freeResources?.length ? 
              `<div class="subsection-title">Free Resources:</div>
               ${data.preparationTips.freeResources.map(resource => 
                 `<div class="list-item">• <strong>${resource.name}:</strong> ${resource.description}</div>`
               ).join('')}` 
              : ''}
            ${data.preparationTips.examStrategy ? 
              `<div class="subsection-title">Exam Strategy:</div>
               <div>${data.preparationTips.examStrategy}</div>` 
              : ''}
          </div>` : ''}
          ...
          <div class="structured-section">
            ...
            <div><strong>Assessment:</strong> ${data.successProbability.percentage}</div>
            ${data.successProbability.factors?.length ? 
              `<div class="subsection-title">Success Factors:</div>
               ${data.successProbability.factors.map(factor => `<div class="list-item">• ${factor}</div>`).join('')}` 
              : ''}
          </div>` : ''}
          ...
          <div class="structured-section">
            ...
            ${data.nextStepsRoadmap.map(step => `
              <div class="roadmap-item">
                <strong>${step.step}</strong><br>
                <small>⏰ ${step.timeline} | 🔥 Priority: ${step.priority}</small>
              </div>
            `).join('')}
          </div>` : ''}
        `;
